...
class FileDeleteAction extends SparkAction implements ContextAction {
  ...
  void _invoke([List<ws.Resource> resources]) {
    ...
    spark.askUserOkCancel(message, okButtonLabel: 'Delete', title: '删除')
        .then((bool val) {
      if (val) {
    ...
    spark.askUserOkCancel(
      ...
      if (val) {
        ...
        project.delete().catchError((e) {
          spark.showErrorMessage("在删除项目错误", exception: e);
        });
      }
    });
  }
...
''', okButtonLabel: 'Delete', title: '从磁盘删除文件').then((bool val) {
      if (val) {
        file.delete().catchError((e) {
          spark.showErrorMessage("在删除文件时出错", exception: e);
        });
      }
    });
  }
